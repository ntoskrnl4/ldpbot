...
async def mc(command: str, message: discord.Message):
    async with message.author.typing():
        ...
        try:
            buffer = io.BytesIO(base64.b64decode(status.favicon.split(",")[1]))  # Decode icon
        except AttributeError:  # no icon?
            no_icon = True
        else:
            no_icon = False
        ...
        embed = discord.Embed(color=0x3D69DE, title=servertitle, description=serverurl)
        if not no_icon: embed.set_thumbnail(url="attachment://icon.png")
        embed = embed.add_field(name="Latency", value=f"{latency} ms")
        embed = embed.add_field(name="Version", value=version)

        for player in status.raw["players"]["sample"]:
            players.append(player["name"])
        if len(players) == 0:
            embed.add_field(name=f"Players ({onlineplayercount}/{maxplayercount})", value="[None]", inline=False)
        else:
            embed.add_field(name=f"Players ({onlineplayercount}/{maxplayercount})", value="\n".join(players), inline=False)
