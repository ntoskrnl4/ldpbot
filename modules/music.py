...
def get_queue_list(queue, length=20):
	...
	for song in queue[:length]:
		duration = "length unknown" if song.duration == 0 else f"{song.duration//60}m{song.duration%60}s"
...
async def command(command: str, message: discord.Message):
	...
	if message.author.id in music_blacklist:
	...
	if parts[1] == "play":
			...
			if not check_if_user_in_channel(vc.channel, message.author.id):
				try:
					...
					return
			if len(guild_queue[message.guild.id]) == 0:
